...
export class EditOverview implements OnInit
{
    ...
    {
        const starting_date = new Date(this.updateFrom.value.starting_date).toISOString().split('T')[0];
        const submission_deadline = new Date(this.updateFrom.value.submission_deadline).toISOString().split('T')[0];
        const winner_announcement_date = new Date(this.updateFrom.value.winner_announcement_date).toISOString().split('T')[0];
