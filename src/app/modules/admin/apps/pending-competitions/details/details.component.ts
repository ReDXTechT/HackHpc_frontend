...
import {TestCompetition} from "../test_competition/test-competition";
...
export class PendingCompetitionsDetailsComponent implements OnInit
{
    ...
    testCompetition(competition: any): void
    {
        const dialogRef  = this._matDialog.open(TestCompetition, {
            autoFocus: false,
            data     : {
                competition : competition
            },
        ...
        dialogRef.afterClosed().subscribe((updatedCompetition) => {
            ...
            if(updatedCompetition){
                this.competitionService.updateCompetitionAMIForTest(competition.id,updatedCompetition).subscribe(res=>{
                    if(res){
                        this.competitionService.testCompetition(competition.id,this.authenticationService.currentUserValue.id).subscribe(res=>{
                            window.open(res.instance_url, '_blank');
                        })
                    }
                })
            }
